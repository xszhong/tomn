...
public class TomnRegex {	
	 ...
	 * */
	public static String MODIFIER_REGEX;
	public static Pattern MODIFIER_PATTERN;
	...
	public static enum TokenType{
		YEAR, YEAR_MID, YEAR_YEAR, MONTH, MONTH_ABBR, MONTH_MID, MONTH_MONTH, YEAR_MONTH, WEEK, WEEK_ABBR, WEEK_WEEK, DATE, TIME, TIME_TIME, DAY_TIME, SEASON, TIMELINE,
		PERIOD, DECADE, BASIC_NUMBER, BASIC_NUMBER_NUMBER, ORDINAL, ORDINAL_ORDINAL, DIGIT, DIGIT_DIGIT, NUMERAL, NUMERAL_NUMERAL, HALFDAY, HALFDAY_HALFDAY,
		TIME_UNIT, DURATION, DURATION_DURATION, HOLIDAY, TIME_ZONE, ERA, MODIFIER, LINKAGE, COMMA
	}
	...
	private static void loadTomnRegex(){
		BufferedReader br = IOProcess.newReader(CONST.REGEX_FILE);
//		BufferedReader br = IOProcess.newReader("resources/tomnregex/TomnRegex.txt");
		String line;
		try {
			while((line = br.readLine()) != null){
				...
				else if(regexName.equals("MODIFIER_REGEX"))
					MODIFIER_REGEX = items[1];
	...
	private static void inducePattern(){
		...
		/**POS: NN, JJ; Type: DURATION*/
		DURATION_REGEX = "(" + DIGIT_REGEX_1 + "|" + DIGIT_REGEX_2 + "|" + BASIC_NUMBER_REGEX_1 + "|" + BASIC_NUMBER_REGEX_2 + "|" + ORDINAL_REGEX_1 + "|" + ORDINAL_REGEX_2 + "|a|an)-?(" + TIME_UNIT_REGEX + ")";
		 ...
		 * Modifiers
		 * */
		MODIFIER_PATTERN = Pattern.compile(MODIFIER_REGEX, Pattern.CASE_INSENSITIVE);
		
		LINKAGE_PATTERN = Pattern.compile(LINKAGE_REGEX, Pattern.CASE_INSENSITIVE);
		COMMA_PATTERN = Pattern.compile(COMMA_REGEX, Pattern.CASE_INSENSITIVE);
	}
}
