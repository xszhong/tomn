...
public class InduceBilouTradAnnotatedTml {
	private ParseTimeML parseTimeML;
	
	public InduceBilouTradAnnotatedTml(){
	...
	public static void main(String[] args) {
		...
		InduceBilouTradAnnotatedTml inducer = new InduceBilouTradAnnotatedTml();
	...
	public void induceAnnotatedTimeML(String inputTmlDir, String crfLogDir, String outputTimeMLDir, String outputShellDir, String evaluationDir){
		...
		sb.append("python TE3-evaluation.py " + inputTmlDir + " " + outputTimeMLDir + " 0 > " + evaluationDir + "result/bilou-trad/" + paths[paths.length - 3] + ".log");
...
//				int tokenEndCharPosition = taggedToken.getEndCharPosition();
				...
				if(predictTag.equals(CONST.BILOU_B)){
					...
					isPreviousTokenTimex = true;
				}else if(predictTag.equals(CONST.BILOU_L)){
					if(! isPreviousTokenTimex){
						timexBeginTokenPosition = tokenPosition;
					}
					timexEndTokenPosition = tokenPosition;
					...
					isPreviousTokenTimex = false;
				}else if(predictTag.equals(CONST.BILOU_U)){
					if(isPreviousTokenTimex && tokenPosition - 1 >= 0){
						timexEndTokenPosition = tokenPosition - 1;
